...
Write-Host "Triggered Agent Stop Action: Target=$TargetId, Initiator=$InitiatorUpn"
...
if ($DryRun) {
    Write-Host "DryRun: Would execute action 'stop' on target agent '$TargetId'."
    return
}

if ($PSCmdlet.ShouldProcess($TargetId, 'Stop')) {
    # Place your live Microsoft Graph / Teams Admin Center API calls here to stop/disable the app/agent.
    Write-Host "Successfully executed action 'stop' on target agent '$TargetId' in Microsoft 365 Tenant."
}
